              ...
              <div className="leftFormContent">
                ...
                <div className="content__body">
                  <dl className="dl">
                    ...
                    by {item.OwnerFirstName + " " + item.OwnerLastName} ({item.OwnerEmail})
                  ...
                  {item !== null && item.votes.length > 0 ?
                    <div className="row">
                      ...
                      <div className="col col--4" >
                        <dl className="dl">
                          <dt>Votes: {item && item.votes.length}</dt>
                          ...
                          {item && item.votes.map((vote) =>
                            <dd key={vote.id}>
                              ...
                              <Rate allowClear={false} defaultValue={parseInt(vote.score) / 5} />    </dd>
                          )}
                        </dl>
                      </div>
                    </div>
                    ...
                    <div className="rightForm">
                      ...
                      <div className="content__body kpnNotesField">
                        <dl className="dl">
                          ...
                          <button onClick={this.voteNow} disabled={this.state.votComment.length < 5} className="button">Vote</button>
                          {this.state.voteNow
                            && <button onClick={() => this.setState({ voteNow: false })} className="button">Cancel</button>}
                        </dl>
                      </div>
                    </div>
                  )}

                {!this.state.voteNow && item.status != "OPEN"
                  && <button onClick={() => this.setState({ voteNow: true })} className="button">Vote</button>}
                {item.status === "OPEN" &&
                  <button onClick={() => this.props.completeIdea("COMPLETE")} className="button">Complete</button>
                }
